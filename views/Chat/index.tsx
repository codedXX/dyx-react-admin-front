...
const Chat: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-140px)]">
      <Card className="h-full flex flex-col p-0 overflow-hidden" title="团队聊天">
        {/* Chat Area */}
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isMe(msg.sender) ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-200 text-slate-600'}`}>
                  <UserIcon size={14} />
                </div>
                <div
                  className={`max-w-[70%] p-3 rounded-lg text-sm ${isMe(msg.sender)
        ...
        <div className="p-4 bg-white border-t border-slate-100 flex gap-3">
          ...
          />
          <Button onClick={sendMessage} variant="primary" className="!px-3" disabled={!isConnected}>
            <Send size={18} />
          </Button>
        </div>
      </Card>
