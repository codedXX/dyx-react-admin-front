      ...
      >
        <div className="overflow-x-auto">
                ...
                <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <img src={user.avatar || 'https://via.placeholder.com/40'} alt="" className="w-8 h-8 rounded-full bg-slate-200" />
                      <span className="font-medium text-slate-700">{user.username}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${user.roleId === 1 ? 'bg-primary-50 text-primary-600' : 'bg-slate-100 text-slate-600'
                      }`}>
                      {user.roleId === 1 ? '管理员' : user.roleId === 2 ? '普通用户' : '访客'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-slate-600">{user.email}</td>
                  <td className="px-4 py-3">
                    <span className={`w-2 h-2 rounded-full inline-block mr-2 ${user.status === 1 ? 'bg-emerald-500' : 'bg-rose-500'
                      }`} />
                    {user.status === 1 ? '正常' : '禁用'}
                  </td>
          ...
          <div className="flex justify-end mt-4 gap-2">
            <Button variant="secondary" disabled={pagination.current === 1} onClick={() => fetchUsers(pagination.current - 1, pagination.pageSize, keyword)}>上一页</Button>
            <span className="py-2 text-sm text-slate-600">第 {pagination.current} 页</span>
            <Button variant="secondary" disabled={users.length < pagination.pageSize} onClick={() => fetchUsers(pagination.current + 1, pagination.pageSize, keyword)}>下一页</Button>
          </div>
        </div>
      </Card>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg w-full max-w-md p-6 shadow-xl">
            <h3 className="text-lg font-semibold mb-4">{modalMode === 'create' ? '新增用户' : '编辑用户'}</h3>
            <Form form={form} onFinish={handleSubmit} layout="vertical">
              <Form.Item name="username" label="用户名" rules={[{ required: true, message: '请输入用户名' }]}>
                <Input placeholder="请输入用户名" />
              </Form.Item>
              <Form.Item name="password" label="密码" rules={[{ required: modalMode === 'create', message: '请输入密码' }]}>
                <Input.Password placeholder={modalMode === 'edit' ? '留空不修改' : '请输入密码'} />
              </Form.Item>
              <Form.Item name="email" label="邮箱" rules={[{ type: 'email', message: '请输入有效的邮箱' }]}>
                <Input placeholder="请输入邮箱" />
              </Form.Item>
              <Form.Item name="roleId" label="角色" initialValue={2}>
                <Select>
